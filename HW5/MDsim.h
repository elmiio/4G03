...
private:
    ...
    //half timestep
    double half_dt = 0.5 * deltaT;
    if (part == 1) {
        //update velocities (half-step) and positions
        for (vector<Atom>::iterator it = Particles.begin(); it != Particles.end(); ++it) {
            it->vel = it->vel + it->acc * half_dt;
            it->r = it->r + it->vel * deltaT;
        }
    } else {
  ...
  void Histogram(double time) {
  
    //only record specified times
    vector<double> targetTimes = {0.02, 0.04, 0.06, 0.08, 0.1, 0.2, 0.4, 1.0};
    if (std::find(targetTimes.begin(), targetTimes.end(), time) != targetTimes.end()) {
      
      //specify filename accoridng to current time
      ...
      for (vector<Atom>::iterator it = Particles.begin(); it != Particles.end(); it++) {
        ...
        //take quadrature sum of x and y components to get total velocity
        double vel = sqrt(it->vel.x*it->vel.x + it->vel.y*it->vel.y);
